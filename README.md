...
手机摄像头 → 红通道 primary 波形 → 心率与 30 秒 HRV 估计，在浏览器本地处理。HRV 每 5 秒更新，在心率卡片中小字号显示。IMU 只记录三轴加速度（含重力）；优先选择独立后置镜头，不启用补光灯。页面底部版本标记：HRV + AI v6。

可选的「本次测量解读」会将首个有效 30 秒窗口的指标摘要发到用户配置的 HTTPS 服务器，返回中文文字。默认关闭，每次打开网页需自行启用，访问口令仅存在当前页面，不写入 localStorage、导出文件或仓库。视频、原始波形、逐帧 RGB、IMU 和设备标识不会发送给模型。

连接设置填写服务器根地址及访问口令，可选「简明状态 / 测量质量 / 休息与恢复参考 / 研究记录」四种解读方式。首次有效 HRV 自动触发一次，后续每 5 秒只更新指标；失败不自动重试。可以手动对同一个首窗口换一种解读方式。切到后台、关闭解读或重新采集会取消浏览器请求；停止采集后仍可等待文字结果。服务器可能已开始计算，取消浏览器请求不等于撤回已发送数据。

心率使用同一 30 秒窗口的平均脉搏率（60000 / 平均 PPI），与页面每秒更新的最近 8 秒心率可能略有差别。LLM 输出仅对应已发送窗口，不是持续实时状态。合成演示不自动上传，只能手动测试并标记为合成数据。模型不是临床诊断工具，30 秒 PRV 与 LF/HF 不足以单独判断血压、疾病、压力或自主神经状态。
...
GitHub 仅托管网页文件，用户拍摄的视频不会发送给 GitHub。摄像头、波形、心率和 HRV 计算独立于可选解读服务器运行；服务器不可用时仍可采集和导出。后端代码、提示词、模型权重与密钥不在网页发布包中。浏览器摄像头接口需要安全上下文和用户授权，参见 [getUserMedia](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia)。
